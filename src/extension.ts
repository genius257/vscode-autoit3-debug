 ...
 *--------------------------------------------------------*/
/*
 * extension.ts (and activateAu3Debug.ts) forms the "plugin" that plugs into VS Code and contains the code that
...
import { activateAu3Debug, workspaceFileAccessor } from './activateAu3Debug';
...
export function activate(context: vscode.ExtensionContext) {
	...
	switch (runMode) {
		case 'server':
			...
			activateAu3Debug(context, new Au3DebugAdapterServerDescriptorFactory());
			break;

		case 'namedPipeServer':
			// run the debug adapter as a server inside the extension and communicate via a named pipe (Windows) or UNIX domain socket (non-Windows)
			activateAu3Debug(context, new Au3DebugAdapterNamedPipeServerDescriptorFactory());
			break;

		case 'external': default:
			// run the debug adapter as a separate process
			activateAu3Debug(context, new DebugAdapterExecutableFactory());
			break;

		case 'inline':
			// run the debug adapter inside the extension and directly talk to it
			activateAu3Debug(context);
			break;
	}
}
...
class Au3DebugAdapterServerDescriptorFactory implements vscode.DebugAdapterDescriptorFactory {
	...
	createDebugAdapterDescriptor(session: vscode.DebugSession, executable: vscode.DebugAdapterExecutable | undefined): vscode.ProviderResult<vscode.DebugAdapterDescriptor> {
		...
		if (!this.server) {
			...
			this.server = Net.createServer(socket => {
				const session = new Au3DebugSession(workspaceFileAccessor);
...
class Au3DebugAdapterNamedPipeServerDescriptorFactory implements vscode.DebugAdapterDescriptorFactory {
